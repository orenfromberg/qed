...
void AnimEval::init (const aiAnimation * pAnim)
{
	mAnim = pAnim;
	mLastTime = 0.0;
...
void AnimEval::Evaluate(double pTime)
{
// extract ticks per second. Assume default value if not given
	...
	for( unsigned int a = 0; a < mAnim->mNumChannels; a++)
	{
		...
		if( channel->mNumPositionKeys > 0)
		{
			...
			unsigned int frame = (time >= mLastTime) ? mLastPositions[a].data[0] : 0;
		...
		if( channel->mNumRotationKeys > 0)
		{
			...
			unsigned int frame = (time >= mLastTime) ? mLastPositions[a].data[1] : 0;
		...
		if( channel->mNumScalingKeys > 0)
		{
			...
			unsigned int frame = (time >= mLastTime) ? mLastPositions[a].data[2] : 0;
