...
SceneAnim::SceneAnim( const aiScene* pScene, size_t pAnimIndex)
{
	mScene = pScene;
	mCurrentAnimIndex = -1;
	mAnimEvaluator = NULL;
...
void SceneAnim::Calculate( double pTime)
{
	// invalid anim
	if( !mAnimEvaluator)
		return;
...
const aiMatrix4x4& SceneAnim::GetLocalTransform( const aiNode* node) const
{
...
const aiMatrix4x4& SceneAnim::GetGlobalTransform( const aiNode* node) const
{
	NodeMap::const_iterator it = mNodesByName.find( node);
	if( it == mNodesByName.end())
		return mIdentityMatrix;
...
SceneAnimNode* SceneAnim::CreateNodeTree( aiNode* pNode, SceneAnimNode* pParent)
{
	// create a node
...
void SceneAnim::UpdateTransforms( SceneAnimNode* pNode, const std::vector<aiMatrix4x4>& pTransforms)
{
...
void SceneAnim::CalculateGlobalTransform( SceneAnimNode* pInternalNode)
{
